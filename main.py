...
def decode_base64_image(base64_string):
    ...
    try:
        ...
        # Flip frame for mirror view (equivalent to cv2.flip(frame, 1))
...
async def predict(data: ImageData):
    ...
    try:
        ...
        # Decode base64 image to NumPy array using PIL instead of OpenCV
        frame = decode_base64_image(data.image)
        
        # Process with MediaPipe (MediaPipe expects RGB format)
...
def handler(data: dict, context: dict):
    return {"message": "Use specific endpoints (/predict, /camera, /clear)"}